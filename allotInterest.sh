...
do 	
	...
	if [[ $agetype = "minor" ]]
	then
		rate=$(grep $agetype Daily_Interest_Rates.txt | cut -d ' ' -f2 | cut -d% -f1 | cat )
	...
	then
		rate=$(grep $restype Daily_Interest_Rates.txt | cut -d ' ' -f2 | cut -d% -f1 | cat )
		rate=$(echo "scale=10;($rate * 0.01)/1" | bc )
		interest=$(echo "scale=10;($rate * $current_balance)/1" | bc )
		balance=$(echo "scale=10;($interest + $balance)/1" | bc )
	fi
	...
	if [[ $Legacy = "legacy" ]]
	then
		rate=$(grep $Legacy Daily_Interest_Rates.txt | cut -d ' ' -f2 | cut -d% -f1 | cat )
		rate=$(echo "scale=10;($rate * 0.01)/1" | bc )
		interest=$(echo "scale=10;($rate * $current_balance)/1" | bc )
		balance=$(echo "scale=10;($interest + $balance)/1" | bc )
	fi
